...
export const PresetSelectorRuntime = memo(function PresetSelectorRuntime() {
  ...
  const [activePresets, setActivePresets] = useState<Map<string, string>>(
    new Map()
  );
  ...
  const filteredHandlers = useMemo(() => {
    return handlers.filter((handler: any) => {
      const matchesSearch =
        !search || handler._path.toLowerCase().includes(search.toLowerCase());
      const matchesMethod =
        selectedMethod === 'all' ||
  ...
  const handlePresetToggle = useCallback(
    (method: string, path: string, presetLabel: string) => {
      if (!mockClient) return;
            ...
            return (
              <div key={endpoint} style={styles.presetsGroup}>
                ...
                >
                  ...
                  >
                    <span style={styles.methodBadge(method)}>
                      {method.toUpperCase()}
                    </span>
                  ...
                  {items.map(({ handler, preset }: any, index: number) => {
                    ...
                    return (
                      <div
                        ...
                        onClick={() =>
                          handlePresetToggle(method, path, preset.label)
                        }
                      >
                          ...
                          <div style={staticStyles.presetInfo}>
                            ...
                            >
                              ...
                              >
                                ...
                                <div style={staticStyles.presetLabel}>
                                  {preset.label}
                                </div>
                              </div>
                              {isActive && (
                                <div style={staticStyles.activeBadge}>
                                  <span style={styles.pulseAnimation}>●</span>{' '}
                                  Active
                                </div>
                              )}
                            </div>
        ...
        <div style={staticStyles.emptyState}>
          ...
          <div style={staticStyles.emptyText}>
            {search
              ...
              : 'No MSW handlers are registered'}
          </div>
        </div>
      )}
    </div>
