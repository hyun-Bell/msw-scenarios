...
export function MockingControls({ onUpdate }: MockingControlsProps) {
  ...
  const getPresetInfo = (presetName: string | null) => {
    if (!presetName)
      return { label: '선택 없음', description: '', color: 'gray' };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h4 className="text-sm font-medium text-gray-700">개별 프리셋 제어</h4>
        <button
          onClick={handleReset}
      ...
      <div className="space-y-4">
        <div className="bg-gray-50 rounded-lg p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            {(() => {
              const userStatus = status.find(
                (s) => s.method === 'get' && s.path === '/api/users'
              );
              const presetInfo = userStatus?.currentPreset
                ? getPresetInfo(userStatus.currentPreset)
                : null;
          ...
          <div className="grid grid-cols-2 gap-2">
            {presetOptions.map((option) => (
              <button
                key={option.value}
                onClick={() =>
                  handlePresetChange('get', '/api/users', option.value)
                }
                className={clsx(
        ...
        <div className="bg-gray-50 rounded-lg p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            {(() => {
              const postStatus = status.find(
                (s) => s.method === 'post' && s.path === '/api/users'
              );
              const presetInfo = postStatus?.currentPreset
                ? getPresetInfo(postStatus.currentPreset)
                : null;
              const color = presetInfo?.color || 'gray';
              const label = presetInfo?.label || 'None';
          ...
          <div className="grid grid-cols-2 gap-2">
            {[
              ...
              },
              {
                value: 'real-api',
                label: '실제 API',
                description: '실제 API 호출',
              },
            ].map((option) => (
              <button
                key={option.value}
                onClick={() =>
                  handlePresetChange('post', '/api/users', option.value)
                }
                className={clsx(
                  'text-left px-3 py-2 text-sm rounded-md border transition-colors',
                  status.find(
